...
| **⚡ 即装即用** | 两步安装，无需额外配置 |
| **📦 轻量高效** | 单文件，不占用额外资源 |
| **📱 跨平台支持** | PC 和安卓共用同一个文件，一处更新，两平台同步受益 |
...
# 切换显示页面（同一页面 → 关闭；不同页面 → 切换）
h script-binding stats/display-page-2-toggle
```

| 命令 | 说明 |
|:-----|:-----|
| `script-binding stats/display-page-1` ~ `display-page-0` | 单次显示指定页面 |
| `script-binding stats/display-page-1-toggle` ~ `display-page-0-toggle` | 切换显示指定页面 |
